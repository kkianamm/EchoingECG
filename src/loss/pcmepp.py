...
class ClosedFormSampledDistanceLoss(nn.Module):
    """Probabilistic Cross-Modal Embedding (PCME) Loss with Closed-Form Sampled Distance.
    Reference:
        - PCME: Learning Robust Cross-Modal Representations using Probabilistic
        Contrastive Loss
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.prob_distance not in {"csd", "wdist"}:
            raise ValueError(
                f'Invalid prob_distance. Expected ("csd", "wdist"), but {prob_distance=}'
            )

    def max_violation_on(self):
        warnings.warn("PCME loss does not support max violation. Nothing happens")
        return
    ...
    def kl_divergence(self, mu, logsigma):
        ...
        if kl_loss > 10000:
            ...
            warnings.warn(
                f"Detected a VIB loss explosion ({kl_loss=} > 10000). Ignore the VIB loss for stability."
            )
            return 0
        return kl_loss
    ...
    def _compute_closed_form_loss(self, input1, input2, matched, smoothness=0):
        ...
        sigma_pdist = (
            torch.exp(input1["std"]).unsqueeze(1) + torch.exp(input2["std"]).unsqueeze(0)
        ).sum(-1)
    ...
    def _compute_wd_loss(self, input1, input2, matched, smoothness=0):
        ...
        mu_pdist = (
            ((input1["mean"].unsqueeze(1) - input2["mean"].unsqueeze(0)) ** 2).sum(-1).view(-1)
        )
        sigma_pdist = (
            (
                (
                    torch.exp(input1["std"] / 2).unsqueeze(1)
                    - torch.exp(input2["std"] / 2).unsqueeze(0)
                )
                ** 2
            )
            .sum(-1)
    ...
    def forward(self, img_emb, cap_emb, matched=None):
        ...
        if self.vib_beta != 0:
            vib_loss = self.kl_divergence(img_emb["mean"], img_emb["std"]) + self.kl_divergence(
                cap_emb["mean"], cap_emb["std"]
            )
        ...
        if self.smoothness_alpha:
            smooth_i2t_loss = loss_fn(
                img_emb, cap_emb, matched=matched, smoothness=self.smoothness_alpha
            )
            smooth_t2i_loss = loss_fn(
                cap_emb, img_emb, matched=matched.T, smoothness=self.smoothness_alpha
            )
            loss = loss + self.smoothness_alpha * (
                smooth_i2t_loss["loss"] + smooth_t2i_loss["loss"]
            )
            loss_dict["loss/loss"] = loss
            loss_dict["loss/n_pseudo_gts"] = (
                smooth_i2t_loss["n_pseudo_gts"] + smooth_t2i_loss["n_pseudo_gts"]
            )

        return loss, loss_dict


# test code with random tensor
if __name__ == "__main__":
    # Create an instance of the loss function with default parameters
    loss_fn = ClosedFormSampledDistanceLoss()

    # Define batch size and embedding dimension
    batch_size = 32
    embedding_dim = 128

    # Create dummy image and caption embeddings with 'mean' and 'std' tensors
    # For testing purposes, we use random tensors

    # Random mean embeddings
    img_mean = torch.randn(batch_size, embedding_dim)
    cap_mean = torch.randn(batch_size, embedding_dim)

    # Random log standard deviation embeddings
    # It's common to initialize log standard deviations close to zero
    img_log_std = torch.randn(batch_size, embedding_dim) * 0.01
    cap_log_std = torch.randn(batch_size, embedding_dim) * 0.01

    # Create img_emb and cap_emb dictionaries
    img_emb = {"mean": img_mean, "std": img_log_std}
    cap_emb = {"mean": cap_mean, "std": cap_log_std}

    # Optional: Create a 'matched' matrix if you have ground-truth matching labels
    # For this test, we'll use an identity matrix indicating perfect matching
    matched = torch.eye(batch_size)

    # Move tensors to GPU if available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_emb = {k: v.to(device) for k, v in img_emb.items()}
    cap_emb = {k: v.to(device) for k, v in cap_emb.items()}
    matched = matched.to(device)
    loss_fn = loss_fn.to(device)

    # Call the loss function
    loss, loss_dict = loss_fn(img_emb, cap_emb)

    # Print the loss and loss_dict
